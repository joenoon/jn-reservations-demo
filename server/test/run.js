...
const ENDPOINT = `https://${API_ID}.execute-api.us-east-1.amazonaws.com/latest/graphql`;
...
async function main() {
  ...
  log(`------ Search reservation with hotelName: Hotel SF`);
  result = await testSearchReservations({hotelName: 'Hotel SF'});
  log(result);

  log(
    `------ Search reservation with hotelName: Hotel SF, arrivalDate: 2018-04-10`
  );
  ...
  log(
    `------ Search reservation with hotelName: Hotel SF, arrivalDate: 2018-04-10, departureDate: 2019-04-11 (none)`
  );
  result = await testSearchReservations({
    hotelName: 'Hotel SF',
    arrivalDate: '2018-04-10',
